### LCVP-Based Species Harmonizing function (LCVP-Bash) - by Michael Wohlwend, 12/20###
species.unifier<- function(names,mdist=2){
  ...
  nam<-gsub(" subsp\\..+","",nam)
  nam<-gsub(" var\\..+","",nam)
  nam<-gsub(" varitas ","",nam) # There are some species named that way, that's why the spaces
  nam<-gsub(" spp\\.+","",nam)
  nam<-gsub(" sp\\.+","",nam)
  ...
  #tab_lcvp comes with the package. full ist form of their dataset
  ...
  for (i in which(tab_lcvpd$Submitted_Name%in%nam)){
  output$Submitted_Name[i]<-unique(names[which(nam==tab_lcvpd$Submitted_Name[i])])[1]
  }
  ...
  diagnostics_output<<-diagnostics # wanted to turn this into an optional output, but as it has to do the calculations anyway, takes too much time for too litte effect. also not sure if it works correctly
